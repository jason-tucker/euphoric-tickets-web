...
export default async function DemoErrorsPage({ searchParams }: { searchParams: Promise<{ level?: string }> }) {
  const persona = getPersona(await getPersonaKey())
  if (!persona.isSudo) return <PersonaGate title="Sudo — Bot errors" need="the bot owner (Sudo persona)" />

  const sp = await searchParams
  const level = ['error', 'warn', 'info'].includes(sp.level ?? '') ? sp.level : undefined
  const rows = getDemoBotErrors(level, new Date())
  const levelColor: Record<string, string> = { error: 'text-red-500', warn: 'text-amber-500', info: 'text-muted-foreground' }
        ...
        <CardContent className="p-0 sm:p-0">
          <Table>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className={`text-xs font-medium uppercase ${levelColor[r.level] ?? ''}`}>{r.level}</TableCell>
                  ...
                  <TableCell className="text-sm">
                    ...
                    {r.context && <pre className="mt-1 overflow-x-auto rounded bg-muted/40 p-1.5 text-[10px] text-muted-foreground">{JSON.stringify(r.context)}</pre>}
                  </TableCell>
                  <TableCell className="hidden text-xs text-muted-foreground sm:table-cell">{relativeTime(r.createdAt)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
